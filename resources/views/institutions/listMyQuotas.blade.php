...
@section('content')
    ...
    <section class="minhas-cotas rifas-section">
        @if ($activatedRaffles->isNotEmpty())
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <p style="color:var(--black);">Você,
                                            <strong>{{ Auth::guard('instituicao')->user()->nome }}</strong>, confirma que
                                            irá
                                            desativar a
                                            <br><strong> Rifa: <span
            ...
            <ul class="titulo-my-quotas">
                <li>
                    <h1>Rifas encerradas de <strong>{{ $instituicaoLogada->nome }}:</strong></h1>
                </li>
            </ul>
            <section class="minhas-cotas rifas-section">
                @foreach ($deactivatedRaffles as $idRifa => $rifas)
                    <div class="rifa-container {{ $rifas->ativado ? '-ativado' : '-desativado' }}">
                        <div class="rifa-top">
                            <div class="rifa-top-top">
                                <img class="rifa-icon" src="{{ asset('img/raffles/' . $rifas->imagem) }}"
                                    alt="Imagem da rifa">
                                <h3 class="rifa-title -quotas">Rifa: {{ $rifas->titulo_rifa }}</h3>
                            </div>
                            <p>Data do sorteio:
                            ...
                            <p>Valor por cota:
                                R$<strong>{{ number_format($valorCotas->get($rifas->id) ?? 0, 2, ',', '.') }}
                                </strong></p>
                            ...
                            <div class="options">
                                <a class="btn btn-secondary"
                                    href="{{ route('redirecionarRifa', ['id' => $rifas->id]) }}">Ver
                                    Rifa</a>
                                <a type class="btn btn-primary"
                            ...
                            <div class="modal fade" id="modalConfirmaDesativação{{ $rifas->id }}" tabindex="-1"
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <p style="color:var(--black);">Você,
                                                <strong>{{ Auth::guard('instituicao')->user()->nome }}</strong>, confirma
                                                que irá
                                                desativar a
                                                <br><strong> Rifa: <span
                                                        style="text-transform:uppercase">{{ $rifas->titulo_rifa }}</span></strong>
        ...
        @else
            <p>Ainda não criou nenhuma rifa!</p>
            <a class="btn btn-secondary" href="{{ route('redirecionarCreateRaffle') }}">Crie uma rifa!</a>
        @endif
    </section>
